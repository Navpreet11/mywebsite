from flask import*
import mysql.connector


data=mysql.connector.connect(host="bwpdytoqstxaoloa7qo1-mysql.services.clever-cloud.com",password="OKv7b1ieip4jHAmq97Cn",user="u7fmqovuckzt7ing",database="bwpdytoqstxaoloa7qo1")
cur=data.cursor()
            
app=Flask(__name__)
app.secret_key="@navpreetsingh1127"

@app.route("/")
def home():

   

    ip = request.headers.get('X-Forwarded-For', request.remote_addr)
    if ip and "," in ip:
        ip = ip.split(",")[0].strip()
    try:
        location = request.get(f"https://ipwho.is/{ip}",timeout=5).json()
    except:
        location = {"status": "fail"}
    di = {
    "ip": ip,
    "country": location.get("country"),
    "region": location.get("region"),
    "city": location.get("city"),
    "lat": location.get("latitude"),
    "lon": location.get("longitude"),
    "isp": location.get("connection", {}).get("isp") }


    cur.execute("insert into info values(%s,%s,%s,%s,%s,%s,%s)",(di["ip"],di["country"],di["region"],di["city"],di["lat"],di["lon"],di["isp"]))
    data.commit()

    
    
    return render_template("home.html")

@app.route("/Skills")
def skills():
    return render_template("skills.html")

@app.route("/education")
def education():
    return render_template("education.html")

@app.route("/contact",methods=["POST","GET"])
def form():
    if request.method=="POST":
        name=request.form["name"]
        email=request.form["email"]
        message=request.form["message"]

        try:
            data=mysql.connector.connect(host="bwpdytoqstxaoloa7qo1-mysql.services.clever-cloud.com",password="OKv7b1ieip4jHAmq97Cn",user="u7fmqovuckzt7ing",database="bwpdytoqstxaoloa7qo1")
            cur=data.cursor()
            cur.execute("insert into users (name,email,message) values(%s,%s,%s)",(name,email,message,))
            data.commit()

            cur.close()

           

            data.close()
           
            return render_template("done.html")
    
        except Exception as e:
            return f"{e} having an error here.you can contact me via my email:ns6685509@gmail.com"
        

        
    return render_template("form.html")

#if __name__=="__main__":
 #  app.run(debug=True,host="0.0.0.0",port="1127")
